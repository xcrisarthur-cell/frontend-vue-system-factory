...
const handleLogin = async () => {
  if (!selectedWorkerId.value || !password.value) {
    await modal.showWarning('Pilih supervisor dan masukkan password')
    return
  }
  ...
  try {
    ...
    // Simpan session supervisor
    supervisorSession.setSupervisor(res.data)
    
    await modal.showSuccess('Login berhasil')
    router.push('/production-supervisor')
  } catch (err) {
    error.value = err.response?.data?.detail || 'Login gagal. Password salah atau supervisor tidak ditemukan.'
  ...
  <div class="login-container">
    <div class="login-card">
      <div class="login-header">
        ...
        <p class="subtitle">Pilih nama supervisor dan masukkan password</p>
      </div>
        ...
        <div class="form-group">
          ...
          >
            <option disabled value="">
              {{ isLoading ? 'Memuat...' : (workers.length === 0 ? 'Tidak ada supervisor' : 'Pilih Supervisor') }}
            </option>
            <option

...
const handleDelete = async (id, code) => {
  const confirmed = await modal.confirm(`Yakin ingin menghapus department "${code}"?`)
  if (!confirmed) return
  
  try {
    await departmentsApi.delete(id)
    await modal.showSuccess('Department berhasil dihapus')
    fetchData()
  } catch (err) {
    await modal.showError(err.response?.data?.detail || 'Gagal menghapus department')
  }
}
...
<template>
  <div class="page-container">
    ...
    <div v-else-if="departments.length === 0" class="empty-state">
      <p>Tidak ada data departments</p>
    </div>
    ...
    <table v-else class="data-table">
      ...
      <tbody>
        <tr v-for="dept in departments" :key="dept.id">
          ...
          <td data-label="Division">{{ getDivisionName(dept.division_id) }}</td>
          <td class="actions" data-label="Aksi">
            <button class="btn-edit" @click="handleEdit(dept.id)">Edit</button>
            <button class="btn-delete" @click="handleDelete(dept.id, dept.code)">Hapus</button>
          </td>
        </tr>
      </tbody>
    </table>

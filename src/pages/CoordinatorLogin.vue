...
const handleLogin = async () => {
  if (!selectedWorkerId.value || !password.value) {
    await modal.showWarning('Pilih coordinator dan masukkan password')
    return
  }
  ...
  try {
    ...
    // Simpan session coordinator
    coordinatorSession.setCoordinator(res.data)
    
    await modal.showSuccess('Login berhasil')
    router.push('/production-coordinator')
  } catch (err) {
    error.value = err.response?.data?.detail || 'Login gagal. Password salah atau coordinator tidak ditemukan.'
